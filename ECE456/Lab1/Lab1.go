...
func main(){
  ...
  //read the keys from Keys.txt  Change Keys.txt to whatever you want to read
  keys := read_file("Keys.txt")
  //this line is equivalent to "var encoded []byte = encode(..." but is cleaner
  //Encoded_data is not a .text file, but you can use cat to see what is in it if you want(not guarenteed to be ascii when encoded)
  ...
  //print the data that we just read in integer value (because it might not be ascii, and we need to check that it changed)
  fmt.Println(data)
