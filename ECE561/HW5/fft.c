...
*/
  ...
  printf("TouchArray %i", int_TouchArray);
  printf("CheckSum %i", int_CheckSum);
  printf("InitX %i", int_InitX);
  printf("int_InitU %i", int_InitU);
  printf("InitU2 %i", int_InitU2);
  printf("BitReverse %i", int_BitReverse);
  printf("FFT1D %i", int_FFT1D);
  printf("TwiddleOneCol %i", int_TwiddleOneCol);
  printf("Scale %i", int_Scale);
  printf("Transpose %i", int_Transpose);
  printf("CopyColumn %i", int_CopyColumn);
  printf("reserve %i", int_Reverse);
  printf("FFT1DOnce %i", int_FFT1D0nce);
  printf("PrintArray %i", int_printArray);
  printf("log_2 %i", int_log_2);
  printf("printerr %i", int_printerr);
  printf("SlaveStart %i", int_slave_start);
  {exit(0);};
}
...
void InitX(double *x)
{
  ...
  for (j=1*rootN/16; j<2*rootN/16; j++) {
    k = j * (rootN + pad_length);
    for (i=0;i<rootN;i++) {
      x[2*(k+i)] = drand48();
      x[2*(k+i)+1] = drand48();
  ...
  for (j=2*rootN/16; j<3*rootN/16; j++) {
    k = j * (rootN + pad_length);
    for (i=0;i<rootN;i++) {
      x[2*(k+i)] = drand48();
      x[2*(k+i)+1] = drand48();
  ...
  for (j=3*rootN/16; j<4*rootN/16; j++) {
    k = j * (rootN + pad_length);
    for (i=0;i<rootN;i++) {
      x[2*(k+i)] = drand48();
      x[2*(k+i)+1] = drand48();
  ...
  for (j=4*rootN/16; j<5*rootN/16; j++) {
    k = j * (rootN + pad_length);
    for (i=0;i<rootN;i++) {
      x[2*(k+i)] = drand48();
      x[2*(k+i)+1] = drand48();
  ...
  for (j=5*rootN/16; j<6*rootN/16; j++) {
    k = j * (rootN + pad_length);
    for (i=0;i<rootN;i++) {
      x[2*(k+i)] = drand48();
      x[2*(k+i)+1] = drand48();
  ...
  for (j=6*rootN/16; j<7*rootN/16; j++) {
    k = j * (rootN + pad_length);
    for (i=0;i<rootN;i++) {
      x[2*(k+i)] = drand48();
      x[2*(k+i)+1] = drand48();
  ...
  for (j=7*rootN/16; j<8*rootN/16; j++) {
    k = j * (rootN + pad_length);
    for (i=0;i<rootN;i++) {
      x[2*(k+i)] = drand48();
      x[2*(k+i)+1] = drand48();
  ...
  for (j=8*rootN/16; j<9*rootN/16; j++) {
    k = j * (rootN + pad_length);
    for (i=0;i<rootN;i++) {
      x[2*(k+i)] = drand48();
      x[2*(k+i)+1] = drand48();
  ...
  for (j=9*rootN/16; j<10*rootN/16; j++) {
    k = j * (rootN + pad_length);
    for (i=0;i<rootN;i++) {
      x[2*(k+i)] = drand48();
      x[2*(k+i)+1] = drand48();
  ...
  for (j=10*rootN/16; j<11*rootN/16; j++) {
    k = j * (rootN + pad_length);
    for (i=0;i<rootN;i++) {
      x[2*(k+i)] = drand48();
      x[2*(k+i)+1] = drand48();
  ...
  for (j=11*rootN/16; j<12*rootN/16; j++) {
    k = j * (rootN + pad_length);
    for (i=0;i<rootN;i++) {
      x[2*(k+i)] = drand48();
      x[2*(k+i)+1] = drand48();
  ...
  for (j=12*rootN/16; j<13*rootN/16; j++) {
    k = j * (rootN + pad_length);
    for (i=0;i<rootN;i++) {
      x[2*(k+i)] = drand48();
      x[2*(k+i)+1] = drand48();
  ...
  for (j=13*rootN/16; j<14*rootN/16; j++) {
    k = j * (rootN + pad_length);
    for (i=0;i<rootN;i++) {
      x[2*(k+i)] = drand48();
      x[2*(k+i)+1] = drand48();
  ...
  for (j=14*rootN/16; j<15*rootN/16; j++) {
    k = j * (rootN + pad_length);
    for (i=0;i<rootN;i++) {
      x[2*(k+i)] = drand48();
      x[2*(k+i)+1] = drand48();
...
void Transpose(long n1, double *src, double *dest, long MyNum, long MyFirst, long MyLast, long pad_length)
{
  ...
  for (l=MyNum+1;l<P;l++) {
    ...
    for (k=0; k<numblks; k++) {
      ...
      for (m=0; m<numblks/4; m++) {
        for (i=0; i<blksize; i++) {
          ...
          for (j=0; j<blksize; j++) {
            ...
            //#3        dest[2*(h*n1p+v)+2] = src[2*(v*n1p+h)+2];also changed loop bounds
            //#3        dest[2*(h*n1p+v)+3] = src[2*(v*n1p+h)+3];
          }
        }
        h_off += blksize;
      }
      for (m=numblks/4; m<numblks/2; m++) {
        for (i=0; i<blksize; i++) {
          v = v_off + i;
          for (j=0; j<blksize; j++) {
            h = h_off + j;
      ...
      for (m=numblks/2; m<3*numblks/4; m++) {
        for (i=0; i<blksize; i++) {
          v = v_off + i;
          for (j=0; j<blksize; j++) {
            h = h_off + j;
...
long log_2(long number)
{
  ...
  //  }
  //}
  while ((cumulative < number) && (out < 50)) {
    if (cumulative == number) {
      break;;
    }
    cumulative = cumulative * 2;
